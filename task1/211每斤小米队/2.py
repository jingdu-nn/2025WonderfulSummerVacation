...
class InteractionNode(Node):
    ...
    def say_text(self, text: str) -> bool:
        ...
        try:
            ...
            # 非阻塞，给一点处理时间（使用独立执行器，避免外部spin冲突）
            t_end = time.time() + 2.0
...
def run_a_warehouse_mission():
    ...
    try:
        ...
        success = robot_controller.imu_forward_distance(0.7, base_vel=0.25)
        if not success:
        ...
        # 按用户规则映射：A1→A2，A2→A1；若都没识别，默认A2
        if recognized_a1 and not recognized_a2:
            print("➡️ 识别到 A1（二维码/文字），改为执行 A2 库路径")
            choose_a2 = True
        elif recognized_a2 and not recognized_a1:
            print("➡️ 识别到 A2（二维码/文字），改为执行 A1 库路径")
            choose_a2 = False
        elif recognized_a1 and recognized_a2:
            print("➡️ 同时包含 A1/A2，默认改为执行 A2 库路径")
            choose_a2 = True
        else:
            print("➡️ 未识别到 A1/A2（二维码/文字均失败），默认执行 A2 库路径")
            choose_a2 = True
...
#使用后退逻辑
            print("18. IMU直走1.3米")
            robot_controller.imu_forward_distance(1.3, base_vel=-0.25)
