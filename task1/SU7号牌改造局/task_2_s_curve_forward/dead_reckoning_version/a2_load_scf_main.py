...
# -*- coding: utf-8 -*-
"""
任务2写死版主程序：A-2装载后的正向S弯

任务流程：
1. 等待触摸传感器触发（三击后脑勺）
2. 站立
3. 向后退100cm
4. A-2路径：向左转115°，向前走85cm，再向右转115°，向前走270cm，向右转115°完成
...
class Task2A2LoadSCF:
    """任务2 A-2装载后正向S弯控制器"""
    
    def __init__(self):
        self.logger = get_logger("Task2A2LoadSCF")
        ...
        self.logger.info("任务2 A-2装载后正向S弯控制器初始化完成")
    ...
    def run(self):
        """执行任务2 A-2装载后正向S弯"""
        self.logger.info("=== 开始执行任务2：A-2装载后正向S弯（写死版） ===")
        ...
        try:
            ...
            if not self._execute_a2_to_s_curve():
                return False
            
            self.logger.info("=== 任务2 A-2装载后正向S弯执行完成 ===")
            return True
    ...
    def _execute_exit_warehouse(self) -> bool:
        ...
        try:
            ...
            # 2. 向后退120cm
            self.logger.info("步骤2：向后退120cm")
            if not self.robot.move_backward(1.2):
                self.logger.error("向后退120cm失败")
                return False
    ...
    def _execute_a2_to_s_curve(self) -> bool:
        """执行A-2到S弯道路径"""
        self.logger.info("--- 第三阶段：A-2到S弯道路径 ---")
...
def main():
    ...
    try:
        ...
        if success:
            print("任务2 A-2装载后正向S弯执行成功！")
        else:
            print("任务2 A-2装载后正向S弯执行失败！")
