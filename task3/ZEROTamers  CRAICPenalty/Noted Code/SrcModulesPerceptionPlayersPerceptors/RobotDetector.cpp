 ...
 *
 * 本文件实现了一个使用神经网络在图像中检测SPL机器人的模块。
 * 大部分代码来自PlayersDeeptector，主要是对Bernd Poppinga代码的适配和新模型的集成。
 *
    ...
    onnxConvModel(&Global::getAsmjitRuntime())
{
  ...
  NeuralNetworkONNX::CompilationSettings onnxSettings; // onnxSettings 实际上被忽略，无需设置
  ...
  if(theCameraInfo.camera == CameraInfo::upper)
  {
    ...
    ASSERT(networkParameters.inputChannels == 1 || networkParameters.inputChannels == 3); // 单通道灰度图或三通道YUV图像
...
{
  model->setInputUInt8(0); // 将uint8图像转换为模型所需的float类型
